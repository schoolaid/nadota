...
class BuildQueryPipe
{
    ...
    protected function applyColumnSelection(IndexRequestDTO $data, Collection $fields): void
    {
        ...
        // Ensure the soft delete column is selected so the resulting models
        // expose deleted_at (used for the deletedAt payload and restore permission).
        if ($data->resource->getUseSoftDeletes()) {
            $deletedAtColumn = $data->modelInstance->getDeletedAtColumn();

            if (!in_array('*', $columns, true) && !in_array($deletedAtColumn, $columns, true)) {
