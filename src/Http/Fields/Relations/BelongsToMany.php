...
class BelongsToMany extends Field
{
    ...
    {
        parent::__construct($name, '', FieldType::BELONGS_TO_MANY->value, static::safeConfig('nadota.fields.belongsToMany.component', 'field-belongs-to-many'));
        ...
        // BelongsToMany should not show on index by default
    ...
    /**
     * BelongsToMany fields don't apply sorting to the parent query.
     *
     * @param Builder $query
    ...
    {
        // BelongsToMany doesn't affect parent query sorting
        return $query;
    }
    ...
    /**
     * Fill method for BelongsToMany.
     * Handles syncing the relationship.
     *
     * @param Request $request
     * @param Model $model
     * @return void
    ...
    {
        // BelongsToMany needs the model to be saved first
        // This is typically handled in afterSave callback
    }
    ...
    public function afterSave(Request $request, Model $model): void
    {
        ...
        if ($this->hasPivotData($values)) {
            ...
            // Simple sync without pivot data
    ...
    protected function prepareSyncWithPivot(array $values): array
    {
        ...
        foreach ($values as $item) {
            if (is_array($item) && isset($item['id'])) {
                $id = $item['id'];
                unset($item['id']);
                $syncData[$id] = $item; // Remaining data is pivot attributes
